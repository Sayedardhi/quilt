...
def draw_bars(canvas, left, top, width, height, n):
    ...
    ind_w=(width-1)//(n-1)
    for i in range(n):
        canvas.draw_line(left + (i * ind_w), top,left + (i * ind_w), top + height-1)
...
def draw_eye(canvas, left, top, width, height, n):
    ...
    ind_w = (width - 1) // (n - 1)
    for i in range(n):
        canvas.draw_line((width)//2 + left, top + (height)//2, left + (i * ind_w), top + height-1)
...
def draw_bowtie(canvas, left, top, width, height, n):
    ...
    for i in range(n):
        if n >= 2:
            canvas.draw_line(left, top + (ind_h *i), left + width, top + height - 1 - (ind_h * i), color='red')
